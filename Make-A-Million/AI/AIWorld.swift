...
//                passed,opener,bidHistory,currentTrick,completedTricks,matchScore}
...
//       passed,bidHistory,trump,misdealEligible,currentTrick,completedTricks,
//       capturedByTeam,matchScore)
//
...
struct Determinizer {
    ...
    /// Reassemble a full GameState consistent with the view. `dealSeed` is
    /// not public; we use a placeholder because no reachable transition from
    /// bidding-onward consults it except a misdeal redeal. `misdealEligible`
    /// is likewise post-bidding-irrelevant.
    private func rebuild(hands: [PlayerID: [Card]]) -> GameState? {
        ...
        return GameState(
            dealSeed: 0,
            dealer: dealerBeforeOpener(view.opener),
            hands: hands,
    ...
    private func dealerBeforeOpener(_ opener: PlayerID) -> PlayerID {
        PlayerID((opener.raw + Seats.count - 1) % Seats.count)
    }
}
