...
struct MonteCarloAgent: PlayerAgent {
    ...
    private func decideBid(_ view: PlayerView, legal: [Move]) -> Move {
        ...
        // Pass is absent from `legal` exactly when the engine is enforcing
        // the family rule "the opener must open on turn one". In that case
        // the correct floor action is the cheapest legal bid, NOT a pass —
        // a weak forced opener is stuck opening at the minimum.
        let passMove = legal.first { $0.isPass }
        let cheapestBid = bidMoves.min { $0.1 < $1.1 }?.0
        let floor = passMove ?? cheapestBid ?? legal[0]
        guard !bidMoves.isEmpty else { return floor }
        ...
        guard n > 0 else { return floor }

        let estimate = valueSum / Double(n)                 // expected gross
        let ceiling  = estimate * difficulty.bidAggression  // TUNE

        // Highest legal bid we can justify. If none is affordable, fall to
        // `floor`: a pass when passing is legal, or — when this seat is the
        // forced opener — the minimum bid. A weak forced opener opening at
        // the minimum is correct under the family rule, not a deadlock.
        let affordable = bidMoves
            .filter { Double($0.1) <= ceiling }
            .max { $0.1 < $1.1 }
        return affordable?.0 ?? floor
    }
